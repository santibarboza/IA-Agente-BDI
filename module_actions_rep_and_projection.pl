...
%,distance(MiPos,TargetPos,Dist),life(me,Life),Life> Dist

action(move(To), [at([agent, me], From), ady(From, To)], [at([agent, me], To)], [at([agent, me], From)]).

action(goto(PosDest), [at([agent, me], PosOrig),atPos([agent,me],MiPos),node(PosDest,TargetPos,_C),distance(MiPos,TargetPos,Dist)], [at([agent, me], PosDest),LifeFinal is Life - Dist,life(me,LifeFinal)], [at([agent, me], PosOrig),life(me,Life),atPos([agent,me],MiPos)]).
...
action(getOroTumba(Tumba), [at(Tumba,Nodotumba),has(Tumba,[gold,G]),at([agent, me], PosOrig),has([agent,me],[potion,Potion])],[at([agent, me],Nodotumba),at([gold,G],Nodotumba)],[has(Tumba,_),at([agent, me], PosOrig),has([agent,me],[potion,Potion])]).

action(cast_spell(open([grave, G], [potion,Potion])), [at([agent, me], Pos),at([grave, G], Pos),has([agent,me],[potion,Potion]), has([grave, G],Obj)],[at(Obj,Pos)],[has([agent,me],[potion,Potion]), has([grave, G],_)]).

action(vaciarHomeEnemigo(Home),[at(Home,NodoHome),at([agent, me], PosOrig),has([agent,me],[potion,Potion]),has(Home,[gold,G])],[has([agente,me],[gold,G]),at([agent,me],NodoHome)],[at([agent, me], PosOrig),has([agent,me],[potion,Potion]),has(Home,[gold,G])]).

action(atacarEnemigo([agent,AgenteEnemigo]),Pre,Add,Del):-
	Pre=[atPos([agent,AgenteEnemigo],PosEnemigo),atPos([agent,me],PosMe),enrango(PosEnemigo,PosMe),life(AgenteEnemigo,L),at([agent,AgenteEnemigo],PosInn),not(at([inn,_Inn],PosInn))],
	Add=[life(AgenteEnemigo,0)],
	Del=[life(AgenteEnemigo,L)].

action(atacarEnemigoLejano([agent,AgenteEnemigo]),Pre,Add,Del):-
	Pre=[atPos([agent,AgenteEnemigo],PosEnemigo),atPos([agent,me],PosMe),enrango(PosEnemigo,PosMe),life(AgenteEnemigo,L)],
	Add=[life(AgenteEnemigo,0)],
	Del=[life(AgenteEnemigo,L)].
	
action(atacarEnemigoLejano([agent,AgenteEnemigo]),Pre,Add,Del):-
	Pre=[atPos([agent,AgenteEnemigo],PosEnemigo),atPos([agent,me],PosMe),not(enrango(PosEnemigo,PosMe)),enrango(PosEnemigo,PosMe2),life(AgenteEnemigo,L)],
	Add=[life(AgenteEnemigo,0),atPos([agent,me],PosMe2)],
	Del=[life(AgenteEnemigo,L), atPos([agent,me],PosMe)].

action(huir(_Enemigo),Pre,Add,Del):-
	Pre=[at([agent,me],PosMe),at([inn,_Inn],PosInn),life(me,L)],
	Add=[at([agent,me],PosInn)],
	Del=[life(me,L),at([agent,me],PosMe)].
	
action(huir(_Enemigo),Pre,Add,Del):-
	Pre=[at([agent,me],PosMe),at([home,Home],PosHome),has([agent,me],[gold,Gold])],
	Add=[at([agent,me],PosHome),has([home,Home],[gold,Gold])],
	Del=[has([agent,me],_),at([agent,me],PosMe)].


action(huir([agent,AgenteEnemigo]),[life(AgenteEnemigo,L)],[life(AgenteEnemigo,0)],[life(AgenteEnemigo,L)]).
...
dynamic_state_rels(InitState):- %Revisar quien utiliza Dynamic_state_reles poruqe claramente lo estamos usando para mal
